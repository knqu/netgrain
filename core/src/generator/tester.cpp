...
public:
    ...
    static void testSim(char *file_name, generator &gen, double max_diff) {
        ...
        // find the average of these sums (divided by init price), and compare with expeccted change
        // assume filename is passed.
        ...
        // we divide by the previous to just get the exponential val of the process
        while (in >> curVal) {
        ...
        std::cout << "average return percent per time delta: " <<  avg << "\n";
        std::cout << "expected percentage return per time delts: " << expected << "\n";

        double percent_diff = abs(avg - expected) / ((avg + expected) * 0.5) * 100;
        std::cout << "The percent difference between your input, and the expected input was: " << percent_diff << "%\n";
